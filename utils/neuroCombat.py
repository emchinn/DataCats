...
def neuroCombat(
    ...
    if not isinstance(covars, pd.DataFrame):
        raise ValueError(
            "covars must be pandas dataframe -> try: covars = pandas.DataFrame(covars)"
        )
    ...
    if ref_batch is None:
        ...
        if ref_indices.shape[0] == 0:
            ...
            covars[:, batch_col] = np.unique(covars[:, batch_col], return_inverse=True)[
                -1
            ]
        else:
            covars[:, batch_col] = np.unique(covars[:, batch_col], return_inverse=True)[
    ...
    info_dict = {
        ...
        "batch_info": [
            list(np.where(covars[:, batch_col] == idx)[0]) for idx in batch_levels
        ],
        "design": design,
    }
    ...
    bayes_data = adjust_data_final(
        s_data, design, gamma_star, delta_star, s_mean, mod_mean, v_pool, info_dict, dat
    )
...
def find_parametric_adjustments(s_data, LS, info_dict, mean_only):
    ...
    for i, batch_idxs in enumerate(batch_info):
        if mean_only:
            gamma_star.append(
                postmean(LS["gamma_hat"][i], LS["gamma_bar"][i], 1, 1, LS["t2"][i])
            )
...
def find_non_parametric_adjustments(s_data, LS, info_dict, mean_only):
    ...
    for i, batch_idxs in enumerate(batch_info):
        ...
        temp = int_eprior(s_data[:, batch_idxs], LS["gamma_hat"][i], LS["delta_hat"][i])
...
def adjust_data_final(
    ...
    for j, batch_idxs in enumerate(batch_info):
        ...
        numer = np.array(
            bayesdata[:, batch_idxs] - np.dot(batch_design[batch_idxs, :], gamma_star).T
        )
...
def neuroCombatFromTraining(dat, batch, estimates):
    """
